...
if (!isset($_GET['id']) || empty($_GET['id'])) {
    ...
    $sql = "
        SELECT ec.name, ec.location, ec.capacity, ec.image, 
               COUNT(e.id) AS evacuee_count, 
               CASE WHEN COUNT(e.id) = 0 THEN 'Inactive' ELSE 'Active' END AS status,
               a.barangay
        FROM evacuation_center ec
        LEFT JOIN evacuees e ON ec.id = e.evacuation_center_id AND e.status = 'Admitted'
    ...
    if ($result->num_rows > 0) {
        ...
        $evacueeCount = htmlspecialchars($row['evacuee_count']);
        ...
        $section->addText("Total Evacuees: " . $evacueeCount, ['name' => 'Arial', 'size' => 12], ['alignment' => 'center']);
    } else {
